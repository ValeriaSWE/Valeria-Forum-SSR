...
import { createRouteData, Meta, useLocation, useParams, useRouteData } from "solid-start"
// import SolidMarkdown from "solid-markdown"
// import Highlight from "solid-highlight"
import { CheckAuthLevel } from "~/lib/user"
...
export function routeData() {
    ...
    const data = createRouteData( async () => {
        const res = await GetPost(params.post, searchParams.get('commentSort') || "createdAt", parseInt(searchParams.get('commentPage') || "1") - 1, parseInt(searchParams.get('commentLimit') || "10"))
        
        const postData = res.data.post
        const data ={...res.data, post: {...postData, createdDate: timeSince(new Date(postData.createdAt).getTime()), likeCount: postData.likes.length, likedByUser: false/*postData.likes.includes(JSON.parse(localStorage.getItem('profile'))?.result._id)*/, pfpRaw: postData.creator.profilePicture, creatorPfp: `data:image/png;base64,${btoa(new Uint8Array(postData.creator.profilePicture.data.data).reduce(function (data, byte) {
...
setting the data to the state. */
    // GetPost(params.post, commentSort(), commentPage() - 1, commentLimit()).then(res => {
    //     const { commentPages, commentPageDict } = res.data
    //     const postData =  res.data.post
    //     setPost({...postData, createdDate: timeSince(new Date(postData.createdAt).getTime()), likeCount: postData.likes.length, likedByUser: false/*postData.likes.includes(JSON.parse(localStorage.getItem('profile'))?.result._id)*/, pfpRaw: postData.creator.profilePicture, creatorPfp: `data:image/png;base64,${btoa(new Uint8Array(postData.creator.profilePicture.data.data).reduce(function (data, byte) {
    //         return data + String.fromCharCode(byte);
    //     }, ''))}`})
    ...
    const postData = useRouteData<typeof routeData>()

    // console.log(rawPostData())

    // const { commentPages, commentPageDict, post } = rawPostData
    ...
    function Creator() {
        return (
            <a class={styles.creatorContainer} href={"/forum/user/" + postData()?.post.creator._id}>
                <div class={styles.creatorImg}>
                    <img src={postData()?.post.creatorPfp} alt="" />
                    <i class={'material-icons ' + styles.verified} data={postData()?.post.creator.role}>verified</i>
                </div>
                <h2 class={styles.creatorName}>{postData()?.post.creator.username}</h2>
                <i class={roleBadge.role} data={postData()?.post.creator.role}>{postData()?.post.creator.role}</i>
            </a>
        )
    }
    ...
    function Comment(props: {
        ...
        const [likedByUser, setLikedByUser] = createSignal(false)// props.comment.likes.includes(JSON.parse(localStorage.getItem('profile'))?.result._id))
                ...
                <div class={styles.postComment} id={props.comment._id}>
                    ...
                    <div class={styles.commentStatitics}>
                        ...
                        <button class={styles.postLikeButton} onClick={() => {
                            ...
                            <i class='material-icons' style={postData()?.post.likedByUser ? "color: var(--color-blue-l);" : "color: inherit;"}>thumb_up</i>
                            <span>{postData()?.post.likeCount}</span>
                        </button>
...
//   `;
    ...
    async function savePost() {
        ...
        const newPostData = await EditPost(params.post, $('#editContent').val()!.toString(), activeTags, "")//JSON.parse(localStorage.getItem('profile'))?.token)
    ...
    function EditPostView() {
        ...
        return (
            <div class={styles.postContent}>
                ...
                <h1 class={styles.title} id="post-title">{postData()?.post.title}</h1>
                {/* https://codepen.io/chriscoyier/pen/XWKEVLy */}
                <div class={styles.growWrap} data-replicated-value={postData()?.post.content}>
                    <textarea class={styles.contentEditing} value={postData()?.post.content} id="editContent" onInput="this.parentNode.dataset.replicatedValue = this.value" onKeyDown={(e) => {if (e.ctrlKey && e.key === 's') {e.preventDefault(); savePost()}}}></textarea>
                </div>
                <div class={styles.imageContainer} id="post-image-container">
                    <For each={postData()?.post.images}>{image =>
            ...
            <div class={styles.inheritPost}>
                ...
                <div class={styles.postStatistics}>
                    <button class={styles.postLikeButton} onClick={() => {
                        ...
                        <i class='material-icons' id={"likes-icon-" + params.post} style={postData()?.post.likedByUser ? "color: var(--color-blue-l);" : "color: inherit;"}>thumb_up</i>
                        <span id={"likes-" + params.post}>{postData()?.post.likeCount} Likes</span>
                    </button>
                    <button>
        ...
        return (
            <div class={styles.commentsControls} style={comments().length == 0 ? "display: none;" : ""}>
                ...
                <div class={styles.commentPageControl}>
                    ...
                    <Show when={commentPage() < postData()?.commentPages} fallback={
    ...
    function CommentPageSelector() {
        return (
            <div class={styles.commentsControls} style={"justify-content: center;" + (postData()?.commentPages < 2 ? "display: none;" : "")}>
                <div class={styles.commentPageControl}>
                    <Show when={commentPage() > 1} fallback={
                        <button class={styles.editFeedIconButton} style="background-color: var(--color-white-m); cursor: unset;"><i class='material-icons'>navigate_before</i></button>
                    }>
                    ...
                    <Show when={postData()?.commentPages < 15} fallback={
                        <>
                        ...
                        <Show when={commentPage() <= 3}>
                            ...
                            <p>...</p>
                            <For each={[postData()?.commentPages - 1]}>{(v, i) =>
                            <PageButton v={v}/>
                            }</For>
                        </Show>
                        <Show when={commentPage() >= postData()?.commentPages - 3}>
                            ...
                            <p>...</p>
                            <For each={[postData()?.commentPages - 3, postData()?.commentPages - 2, postData()?.commentPages - 1]}>{(v, i) =>
                            <PageButton v={v}/>
                            }</For>
                        </Show>
                        <Show when={commentPage() < postData()?.commentPages - 3 && commentPage() > 3}>
                            <For each={[0]}>{(v, i) =>
                            <PageButton v={v}/>
                            }</For>
                            <p>...</p>
                        ...
                        <For each={[... Array(postData()?.commentPages).keys()]}>{(v, i) =>
                        <PageButton v={v}/>
                        }</For>
                    </Show>
                    <Show when={commentPage() < postData()?.commentPages} fallback={
