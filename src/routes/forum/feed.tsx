...
export function routeData() {
	...
	const data = createRouteData(async () => {
		...
		const rawAllPosts = await GetAllPosts(
...
function Feed() {
	...
	function FeedContainer(props: { children: any }) {
		return (
			<>
				<div class={styles.feed}>{props.children}</div>
			</>
		);
	}
			...
			<>
				<div class={styles.editFeed}>
					...
					<div class={styles.searchFeed}>
						...
						<input
							type='text'
							placeholder='Sök efter Feed'
							use:model={[search, setSearch]}
							onkeydown={(e) => InputKeyPress(e)}
						/>
					</div>
					<A
						class={styles.editFeedIconButton}
						href='/forum/newpost' /*onclick={() => location.href="/forum/newpost"}*/
					>
		...
		return (
			<>
				<div class={styles.pageControls} style='justify-content: center;'>
					...
					>
						<For each={[...Array(postsRaw()?.pages).keys()]}>
							{(v, i) => <PageButton v={v} />}
						</For>
					</Show>
					<Show
