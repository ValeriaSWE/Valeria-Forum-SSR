...
export default function PostPreview(props: {
	...
	const ShowRoleInPost = (props: { role: string }) => {
		return (
			<>
				<span class={styles.showRole}>
					{/* {props.roleRank >= 5 ? <i class='material-icons'>verified</i> : <></>} */}
					<i class={"material-icons " + styles.verified} data={props.role}>
						verified
					</i>
					<i class={roleBadge.role} data={props.role}>
						{props.role}
					</i>
				</span>
			</>
			...
			<>
				<div class={styles.postStatitics}>
					...
					<p class={styles.postDate}>
						{timeSince(new Date(props.date).getTime())} sedan
					</p>
					<button
						onClick={() => {
							LikePost(
	...
	const profilePicture = `data:image/png;base64,${btoa(
		new Uint8Array(props.data.creator.profilePicture.data.data).reduce(
			function (data, byte) {
	...
	return (
		<>
			<div class={styles.post} id={props.data._id}>
				<A
					href={"/forum/user/" + props.data.creator._id}
					class={styles.postCreatorImg}
				>
				...
				<A
					...
					href={`/forum/post/${props.data._id}`}
				...
				<PostStatitics
					...
					images={props.data.images.length}
				/>
			</div>
		</>
	);
