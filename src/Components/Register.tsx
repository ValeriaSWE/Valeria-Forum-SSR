			...
			<form onSubmit={(e) => e.preventDefault()}>
				<div class={styles.registerInnerContainer}>
					...
					<TextField
						...
						type='email'
						error={emailError()}
						helperText={emailError() ? "Ange en giltlig epost." : ""}
						onChange={(e) => {
					...
					<TextField
						...
						type='password'
						error={passError()}
						helperText={passError() ? "Minst åtta tecken." : ""}
						onChange={(e) => {
					...
					<TextField
						...
						type='password'
						error={passConfError()}
						helperText={passConfError() ? "Matchar inte." : ""}
						onChange={(e) => {
							if (
								e.target.value != $("#register-password").val() &&
								!passConfError()
							) {
...
const registerSubmit = async () => {
	...
	try {
		const { data } = await RegisterUserServerPost(
			email,
			password,
			passwordConfirm,
			username
