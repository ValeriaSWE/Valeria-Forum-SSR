...
import { Auth, CheckAuthLevel } from "../lib/user";
...
export default function UserInfo() {
	...
	GetUserInfo(id).then((res) => {
		...
		setUser({
			...data,
			pfpRaw: data.profilePicture,
			profilePicture: `data:image/png;base64,${btoa(
				new Uint8Array(data.profilePicture.data.data).reduce(function (
					data,
					byte
				) {
					...
					<div class={styles.editModal}>
						...
						<div class={styles.profileContent}>
							<div class={styles.profilePictureDiv}>
								<label for='file'>
									<img
										id={styles.editProfilePictureModal}
							...
							<div class={styles.profileInformation}>
								<TextField
									...
									error={userError()}
									helperText={userError() ? "Minst fyra tecken." : ""}
									onChange={(e) => {
						...
						<div class={styles.editModalSubmit}>
							<button class={styles.saveBtn} id='edit-submit' onClick={save}>
								Spara Profil
							</button>
							<button
	...
	const ProfileInfo = () => {
				...
				<>
					<Show
						when={
							...
							JSON.parse(localStorage.getItem("profile"))?.result.username ===
								user.username
						}
						fallback={
			...
			return (
				<>
					<Skeleton
						variant='circular'
						height='100%'
						style='aspect-ratio: 1/1; margin-right: 1.25rem;'
					/>
					<div class={styles.profileInformation}>
		...
		const FormatDateJoined = (date: string) => {
			...
			return `${formatDate[0]} ${monthNames[parseInt(formatDate[1]) - 1]} ${
				formatDate[2]
			}`;
		};
		return (
			<>
				<div class={styles.userInfo}>
					...
					<Show when={user.username != "username"} fallback={UserLoader}>
						<img
							class={styles.userProfilePicture}
							src={user.profilePicture}
							alt=''
							srcset=''
						/>
								...
								<div class={styles.userProfileWrapper}>
									...
									<div class={styles.usernameAndRoles}>
										...
										<span class={styles.showRole}>
											...
											<i class={roleBadge.role} data={user.role}>
												{user.role}
											</i>
										</span>
										<div class={styles.nicknames}>
											<For each={user.nicknames}>
	...
	const Tab = () => {
		const TabLink = (props: { label: string; link: string }) => {
			return (
				<div
	...
	async function sortComments() {
		...
		setUser({
			comments: { ...user.comments, pages: pages, comments: comments },
		});
	...
	function PageButtonPost(props: { v: number }) {
		const v = props.v;

		return (
			<Show
				when={v + 1 == user.posts.page}
				fallback={
					<button
						class={styles.editFeedIconButton}
						onClick={() => {
							setUser({ posts: { ...user.posts, page: v + 1 } });
							sortPosts();
						}}
					>
						{v + 1}
	...
	function PageButtonComment(props: { v: number }) {
		const v = props.v;

		return (
			<Show
				when={v + 1 == user.comments.page}
				fallback={
					<button
						class={styles.editFeedIconButton}
						onClick={() => {
							setUser({ comments: { ...user.comments, page: v + 1 } });
							sortComments();
						}}
					>
						{v + 1}
	...
	function PostControls() {
		return (
			<div
				class={styles.postsControls}
				style={user.posts.posts.length == 0 ? "display: none;" : ""}
			>
					...
					<button
						...
						onClick={() => {
							if (user.posts.sort == "interactionCount") {
								setUser({
									posts: { ...user.posts, sort: "interactionCount-inverse" },
								});
							} else {
					...
					<button
						...
						onClick={() => {
							if (user.posts.sort == "createdAt") {
								setUser({
									posts: { ...user.posts, sort: "createdAt-inverse" },
								});
							} else {
				...
				<div class={styles.postPageControl}>
					...
					>
						<button
							...
							onClick={() => {
								setUser({
									posts: { ...user.posts, page: user.posts.page - 1 },
								});
								sortPosts();
							}}
						>
					...
					<button class={styles.editFeedIconButton} style='cursor: unset;'>
						Sida: {user.posts.page}
					</button>
					<Show
						when={user.posts.page < user.posts.pages}
						fallback={
							<button
								class={styles.editFeedIconButton}
								style='background-color: var(--color-white-m); cursor: unset;'
		...
		return (
			<div
				...
				style={
					...
					(user.posts.pages < 2 ? "display: none;" : "")
				}
			>
				<div class={styles.postPageControl}>
					<Show
					...
					<Show
						...
						fallback={
							<>
								...
								<Show
									when={
										user.posts.page < user.posts.pages - 3 &&
										user.posts.page > 3
									}
								>
									<For each={[0]}>{(v, i) => <PageButtonPost v={v} />}</For>
					...
					>
						<For each={[...Array(user.posts.pages).keys()]}>
							{(v, i) => <PageButtonPost v={v} />}
						</For>
					</Show>
					<Show
	...
	function CommentControls() {
		return (
			<div
				...
				style={user.comments.comments.length == 0 ? "display: none;" : ""}
			>
					...
					<button
						...
						onClick={() => {
							if (user.comments.sort == "interactionCount") {
								...
								setUser({
									comments: { ...user.comments, sort: "interactionCount" },
								});
							}
					...
					<button
						...
						onClick={() => {
							if (user.comments.sort == "createdAt") {
								setUser({
									comments: { ...user.comments, sort: "createdAt-inverse" },
								});
							} else {
				...
				<div class={styles.commentPageControl}>
					...
					>
						<button
							...
							onClick={() => {
								setUser({
									comments: { ...user.comments, page: user.comments.page - 1 },
								});
								sortComments();
							}}
						>
					...
					<button class={styles.editFeedIconButton} style='cursor: unset;'>
						Sida: {user.comments.page}
					</button>
					<Show
						when={user.comments.page < user.comments.pages}
						fallback={
							<button
								class={styles.editFeedIconButton}
								style='background-color: var(--color-white-m); cursor: unset;'
	...
	function CommentPageSelector() {
		return (
			<div
				...
				style={
					"justify-content: center;" +
					(user.comments.pages < 2 ? "display: none;" : "")
				}
			>
				<div class={styles.commentPageControl}>
					<Show
					...
					<Show
						when={user.comments.pages < 15}
						fallback={
							<>
								{/* Make style work when there is alot of commentPages */}
								<Show when={user.comments.page <= 3}>
									<For each={[...Array(3).keys()]}>
										{(v, i) => <PageButtonComment v={v} />}
									</For>
									<p>...</p>
									<For each={[user.comments.pages - 1]}>
										{(v, i) => <PageButtonComment v={v} />}
									</For>
								</Show>
								<Show when={user.comments.page >= user.comments.pages - 3}>
									<For each={[0]}>{(v, i) => <PageButtonComment v={v} />}</For>
									<p>...</p>
									<For
										each={[
											user.comments.pages - 3,
											user.comments.pages - 2,
											user.comments.pages - 1,
										]}
									>
										{(v, i) => <PageButtonComment v={v} />}
									</For>
								...
								<Show
									when={
										user.comments.page < user.comments.pages - 3 &&
										user.comments.page > 3
									}
								>
									<For each={[0]}>{(v, i) => <PageButtonComment v={v} />}</For>
									<p>...</p>
					...
					>
						<For each={[...Array(user.comments.pages).keys()]}>
							{(v, i) => <PageButtonComment v={v} />}
						</For>
					</Show>
					<Show
	...
	function Comment(props: { comment: any }) {
		const [likedByUser, setLikedByUser] = createSignal(
			props.comment.likes?.includes(
				JSON.parse(localStorage.getItem("profile"))?.result._id
			) || false
		);
		const [likeCount, setLikeCount] = createSignal(
			...
			<>
				<div class={styles.postComment} id={props.comment._id}>
					...
					<A
						...
						href={`/forum/post/${props.comment.onPost}?comment=${props.comment._id}`}
					>
		...
		<>
			...
			<div class={styles.userPosts}>
				<Switch>
					<Match when={activeTab() === "posts"}>
						...
						<div class={styles.posts} id='posts'>
							<Show when={postsLoaded()} fallback={Loader}>
								<For each={user.posts.posts}>
									{(post) => <PostPreview data={post} />}
								</For>
							</Show>
						</div>
						<PostPageSelector />
					...
					<Match when={activeTab() === "comments"}>
						...
						<div class={styles.comments} id='comments'>
							<Show when={commentsLoaded()} fallback={Loader}>
								<For each={user.comments.comments}>
					...
					<Match when={activeTab() === "about"}>{user.about}</Match>
				</Switch>
			</div>
			{/* </Show> */}
		</>
...
export function timeSince(date) {
	...
	if (interval > 1) {
		return (
			Math.floor(interval) + " månad" + (Math.floor(interval) == 1 ? "" : "er")
		);
	}
	interval = seconds / 86400;
	if (interval > 1) {
		return (
			Math.floor(interval) + " dag" + (Math.floor(interval) == 1 ? "" : "ar")
		);
	}
	interval = seconds / 3600;
	if (interval > 1) {
		return (
			Math.floor(interval) + " timm" + (Math.floor(interval) == 1 ? "e" : "ar")
		);
	}
	interval = seconds / 60;
	if (interval > 1) {
		return (
			Math.floor(interval) + " minut" + (Math.floor(interval) == 1 ? "" : "er")
		);
	}
	return (
		Math.floor(seconds) + " sekund" + (Math.floor(interval) == 1 ? "" : "er")
	);
}
